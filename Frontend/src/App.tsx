...
const MorphingGraphic = () => {
  ...
  // 15 Content Bars
  const content = [
    { x: 40, y: 50, w: 60, h: 60, r: "30px", color: "#a78bfa" }, // Avatar
    { x: 120, y: 60, w: 140, h: 14, r: "4px", color: "#f5f5f5" }, // Name
    { x: 120, y: 85, w: 80, h: 8, r: "4px", color: "#737373" }, // Title

    { x: 40, y: 140, w: 240, h: 6, r: "3px", color: "#737373" },
    { x: 40, y: 160, w: 220, h: 6, r: "3px", color: "#737373" },
    { x: 40, y: 180, w: 190, h: 6, r: "3px", color: "#737373" },

    { x: 40, y: 230, w: 90, h: 10, r: "4px", color: "#f5f5f5" },
    { x: 40, y: 255, w: 240, h: 6, r: "3px", color: "#737373" },
    { x: 40, y: 275, w: 230, h: 6, r: "3px", color: "#737373" },
    { x: 40, y: 295, w: 200, h: 6, r: "3px", color: "#737373" },

    { x: 40, y: 345, w: 90, h: 10, r: "4px", color: "#f5f5f5" },
    { x: 40, y: 370, w: 240, h: 6, r: "3px", color: "#737373" },
    { x: 40, y: 390, w: 210, h: 6, r: "3px", color: "#737373" },
    { x: 40, y: 410, w: 160, h: 6, r: "3px", color: "#737373" },

    { x: 40, y: 450, w: 100, h: 6, r: "3px", color: "#737373" },
  ];
...
const LaptopMockup = ({ children }: { children: React.ReactNode }) => (
  <div className="relative mx-auto w-full max-w-[800px]">
    {/* Laptop Screen */}
    <div className="relative bg-neutral-800 rounded-t-xl border-[6px] border-neutral-800 shadow-2xl">
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-16 h-3 bg-neutral-900 rounded-b-md flex justify-center items-center z-20">
        <div className="w-1 h-1 rounded-full bg-neutral-700" />
      </div>
      <div className="relative bg-neutral-950 rounded-lg overflow-hidden aspect-[16/10]">
        {children}
      </div>
    </div>
    {/* Laptop Base */}
    <div className="relative bg-neutral-700 h-3 rounded-b-2xl shadow-xl flex justify-center">
      <div className="w-1/4 h-1 bg-neutral-600 rounded-b-sm" />
    </div>
  </div>
);
  ...
  return (
    <div className="w-full h-full bg-[#0a0a0a] flex flex-col relative overflow-hidden">
      ...
      <div className="flex justify-center pt-3 pb-2 shrink-0">
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-neutral-800/80 border border-neutral-700/50 text-[10px] sm:text-xs text-neutral-300 font-mono font-semibold">
      ...
      <div className="flex-1 flex gap-3 px-3 pb-2 min-h-0">
        ...
        <div className="flex-1 bg-neutral-900/60 rounded-xl border border-neutral-800/60 relative overflow-hidden flex flex-col items-center justify-center">
          {/* Label */}
          <div className="absolute top-2 left-2 flex items-center gap-1.5 px-2 py-1 rounded-full bg-neutral-800/90 border border-neutral-700/50">
            <div className="w-1.5 h-1.5 rounded-full bg-purple-400" />
            <span className="text-[9px] text-neutral-300 font-semibold">Enrollment Interview • Prof. Maxiel</span>
          </div>
        ...
        <div className="w-[42%] bg-neutral-900/60 rounded-xl border border-neutral-800/60 flex flex-col overflow-hidden">
          <div className="px-3 py-2 border-b border-neutral-800/60 shrink-0">
            <span className="text-[9px] sm:text-[10px] font-bold text-neutral-400 uppercase tracking-widest">Your Responses</span>
          </div>
          <div className="flex-1 overflow-hidden px-2 py-2 flex flex-col gap-2 justify-end">
            {responses.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full gap-1 opacity-40">
                ...
                <span className="text-[9px] text-neutral-500 text-center">Respond to Prof. Maxiel's questions.</span>
              </div>
            ) : (
              responses.map((r, i) => (
                <motion.div
                  ...
                  className="bg-neutral-800/60 rounded-lg px-2 py-1.5 border border-neutral-700/40"
                >
                  <div className="flex items-center gap-1 mb-0.5">
                    <div className="w-3 h-3 rounded-full bg-violet-500/20 border border-violet-500/40 flex items-center justify-center">
                      <span className="text-[6px] text-violet-400 font-bold">U</span>
                    </div>
                    <span className="text-[8px] text-neutral-500 font-medium">You</span>
                  </div>
                  <p className="text-[8px] sm:text-[9px] text-neutral-300 leading-relaxed line-clamp-3">{r}</p>
                </motion.div>
              ))
            )}
          </div>
          <div className="px-3 py-2 border-t border-neutral-800/60 shrink-0">
            <span className="text-[8px] text-neutral-600">{responses.length} / 5 Responses Recorded</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-center gap-3 pb-3 shrink-0">
        {/* Mic */}
        <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-2xl flex items-center justify-center shadow-lg transition-all duration-300 ${isListening ? 'bg-emerald-500 shadow-emerald-500/30' : 'bg-neutral-800'}`}>
          {isListening ? (
            ...
            <Mic className="w-4 h-4 sm:w-5 sm:h-5 text-neutral-400" />
          )}
        ...
        {/* Leave */}
        <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-neutral-800 border border-neutral-700 flex items-center justify-center text-neutral-400">
...
const FeatureCard = ({ icon, title, description }: { icon: React.ReactNode, title: string, description: string }) => (
  <motion.div
    whileHover={{ y: -5 }}
    className="h-full bg-neutral-900/50 border border-neutral-800/50 p-6 rounded-2xl flex gap-4 items-start hover:bg-neutral-800/50 transition-colors"
  >
    <div className="p-3 bg-neutral-800 rounded-xl border border-neutral-700/50 flex-shrink-0">
      {icon}
    </div>
    <div>
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-neutral-950 text-neutral-50 font-sans selection:bg-violet-500/30 overflow-x-hidden">
      ...
      <main className="relative z-10 pt-24 pb-16">
        ...
        <section className="container mx-auto px-6 pt-12 pb-24 md:pt-24 md:pb-32">
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            ...
            >
              ...
              <div className="flex flex-col sm:flex-row gap-4 mt-4">
                ...
                <button 
                  ...
                  className="px-8 py-4 rounded-xl bg-neutral-800 hover:bg-neutral-700 text-neutral-200 font-semibold transition-all flex items-center justify-center gap-2 group border border-neutral-700"
                >
            ...
            >
              <div className="absolute inset-0 bg-violet-500/20 blur-[100px] rounded-full opacity-50" />
              <MorphingGraphic />
            </motion.div>
          </div>
        </section>
        ...
        <section id="campus-map" className="container mx-auto px-6 py-24 border-t border-neutral-800/50">
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            <motion.div
              initial={{ opacity: 0, x: -30 }}
            ...
            >
              <div>
                ...
                <div className="mt-6">
                  <a 
                    ...
                    className="inline-flex items-center gap-2 text-violet-400 font-bold hover:text-violet-300 transition-colors uppercase tracking-widest text-xs"
                  >
                    Get Directions
              ...
              <div className="grid sm:grid-cols-2 gap-6">
                <div className="flex gap-4 items-start p-4 rounded-xl bg-neutral-900/50 border border-neutral-800/50">
                  <div className="w-10 h-10 rounded-lg bg-violet-500/10 flex items-center justify-center shrink-0">
                    <MapPin className="w-5 h-5 text-violet-400" />
                  </div>
                  <div>
                    <h4 className="text-sm font-bold text-neutral-200 mb-1">Address</h4>
                    <p className="text-xs text-neutral-400 leading-relaxed">San Nicolas, Sitio Baring,<br />Castillejos, Zambales</p>
                  </div>
                </div>
                <div className="flex gap-4 items-start p-4 rounded-xl bg-neutral-900/50 border border-neutral-800/50">
                  ...
                  <div>
                    <h4 className="text-sm font-bold text-neutral-200 mb-1">Email</h4>
                    <p className="text-xs text-neutral-400 leading-relaxed">prmsucaste1@gmail.com</p>
                  </div>
                </div>
              </div>
            </motion.div>
            ...
            <motion.div
              ...
              viewport={{ once: true }}
              transition={{ duration: 0.8 }}
              className="relative rounded-[2.5rem] overflow-hidden border border-neutral-800 shadow-2xl h-[400px] group"
            >
              <div className="absolute inset-0 bg-violet-500/10 z-10 pointer-events-none group-hover:opacity-0 transition-opacity duration-500" />
              <iframe
      ...
      {/* Footer */}
      <footer className="bg-neutral-950 py-8 relative z-10">
        <div className="container mx-auto px-6">
          <div className="pt-8 border-t border-neutral-800/50 flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-4">
              <a href="https://www.facebook.com/PRMSUCCITCastillejos" target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-neutral-900 border border-neutral-800 flex items-center justify-center hover:bg-neutral-800 hover:border-neutral-700 transition-all group">
                <Facebook className="w-4 h-4 text-neutral-400 group-hover:text-violet-400" />
              </a>
              <a href="https://github.com/Qusaki/Career-Edge-AI" target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-neutral-900 border border-neutral-800 flex items-center justify-center hover:bg-neutral-800 hover:border-neutral-700 transition-all group">
                <Github className="w-4 h-4 text-neutral-400 group-hover:text-violet-400" />
              </a>
            </div>
          </div>
        </div>
