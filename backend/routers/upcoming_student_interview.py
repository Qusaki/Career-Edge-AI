...
def get_evaluation_score_keys(department: str) -> list[str]:
    ...
    if dep == "CTE":
        return [
            "subject_matter_score", "teaching_aptitude_score", "communication_score",
            "motivation_score", "academic_preparedness_score", "problem_solving_score",
            "leadership_score",
        ]
    if dep == "CBAPA":
        return [
            "business_fundamentals_score", "analytical_score", "communication_score",
            "entrepreneurial_score", "academic_preparedness_score", "leadership_score",
            "ethical_score",
        ]
    return [
        "technical_score", "problem_solving_score", "coding_score",
        "communication_score", "soft_skills_score",
    ]


def get_stored_chat_messages(db: Session, session_id: int) -> list[dict[str, str]]:
    history = db.query(UpcomingStudentInterviewMessage).filter(
        UpcomingStudentInterviewMessage.session_id == session_id
    ).order_by(UpcomingStudentInterviewMessage.timestamp.asc()).all()
    return [
        {
...
def start_interview(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    """Starts or resumes the active structured upcoming student interview session."""
...
async def interview_chat_ws(
    ...
    """Handles real-time bi-directional streaming with the configured AI service."""
    if not current_user.department or current_user.department.upper() not in ["CCIT", "CTE", "CBAPA"]:
    ...
    try:
            ...
            if "text" in msg:
                try:
                    ...
                    if is_initial_turn:
                        ...
                        db.add(UpcomingStudentInterviewMessage(
                        ...
                        await websocket.send_json({
                            "type": "error",
                            "message": "No interview response was provided.",
                            "retryable": True,
                        })
                        continue
                    ...
                    async for content in ai_provider.stream_chat(
                        provider_messages, workload="enrollment_interview"
                    ):
                    ...
                    logger.warning(
                        "Enrollment AI stream failed (session_id=%s, error=%s)",
                        session.id,
                        type(error).__name__,
                    )
                    db.rollback()
        ...
        logger.warning(
            "Enrollment WebSocket failed (session_id=%s, error=%s)",
            session.id,
            type(error).__name__,
        )
        try:
...
async def complete_interview(session_id: int, request: UpcomingStudentCompleteInterviewRequest, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    if not current_user.department or current_user.department.upper() not in ["CCIT", "CTE", "CBAPA"]:
        raise HTTPException(status_code=403, detail="Forbidden: This interview simulation is only available to CCIT, CTE, and CBAPA students.")
    ...
    if not history and request.conversation:
        ...
        db.commit()
        history = db.query(UpcomingStudentInterviewMessage).filter(
            UpcomingStudentInterviewMessage.session_id == session.id
        ).order_by(UpcomingStudentInterviewMessage.timestamp.asc()).all()
    if not history:
    ...
    try:
        ...
        response = await collect_ai_response(
            provider,
            [
                {"role": "system", "content": get_evaluation_system_prompt(current_user.department)},
                {
                    "role": "user",
                    "content": (
                        "Evaluate this completed interview transcript. Return only the required "
                        f"JSON object.\n\n{transcript}"
                    ),
                },
            ],
        ...
        evaluation = parse_evaluation_response(
            response, get_evaluation_score_keys(current_user.department)
        )
        ...
        db.rollback()
        logger.warning(
            "Enrollment evaluation failed (session_id=%s, error=%s)",
            session.id,
            type(error).__name__,
        )
        raise HTTPException(
