...
const regex = /  const finishThesisSession = async \(\) => \{[\s\S]*?(?=  const sendToGemini = async \(text: string\) => \{)/;

const replacement = `  const finishThesisSession = async () => {
...
\${thesisConversationLog.map(m => m.sender.toUpperCase() + ": " + m.text).join('\\n')}\`;
         ...
         try {
           ...
           // Map standard LLM output to CCIT specifically for the demo purposes (since we used fallback above)
...
if (regex.test(content)) {
    ...
    console.log("Failed to find finishThesisSession target");
}
